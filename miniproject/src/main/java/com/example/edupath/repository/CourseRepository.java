...
import jakarta.annotation.PostConstruct; // Đã đổi sang jakarta
...
public class CourseRepository {
    ...
    @PostConstruct
    public void init() {
        courses.add(new Course("IELTS-6.5", "IELTS Master", "Advanced", 8500000.0, "Lộ trình đạt 6.5+", "Mr. Anh", "6 tháng", true, 20, LocalDate.of(2023, 12, 1)));
    ...
    public Course findById(String id) { return courses.stream().filter(c -> c.getId().equals(id)).findFirst().orElse(null); }
    public void deleteById(String id) { courses.removeIf(c -> c.getId().equals(id)); }
}